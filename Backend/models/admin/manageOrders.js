...
var item = {
    ...
    customerDetails(id, callback) {
        return db.query('select t2.Email,t1.FirstName,t1.LastName,t1.Image,t1.DOB,t1.ContactNo,t1.AlternateNo from tbluser as t1 join tbluserlogin as t2 on t1.UserID = t2.UserID where t1.UserID = ?', [id], callback);
    },
    ...
    changeShipDateTime(data, callback) {
        return db.query('update tblorder set ShipmentDateTime = ? where OrderID = ?', [data.date, data.id], callback);
    }
};
